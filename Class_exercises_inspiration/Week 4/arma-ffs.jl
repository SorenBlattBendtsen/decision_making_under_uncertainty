...
#Calculate distrance matrix (euclidean distance)
...
#Num of reduces scenarios
num_reduced = 5
#Initialize probabilities as equiprobable
probabilities = repeat([1.0/scenarios], 1, scenarios)[1,:]
...
#Prepare reduced scenario data for plotting
reduced_data = zeros(Float64, steps+1, scenarios)
for i = 1:num_reduced
    reduced_data[:,i] = samples[:,result[2][i]]
end

#Plot
plotdata2 = hcat(range(0,steps;step=1), reduced_data)
plot!(plotdata2[:,1],plotdata2[:,2:num_reduced+1], legend=false, color=:auto)
