...
det_demand = [demand[p][1] for p in P]


#Declare Gurobi model
production_model_det = Model(Gurobi.Optimizer)



#Definition of variables with lower bound 0
@variable(production_model_det, 0<=y[m in M],Int)
@variable(production_model_det, 0<=x[m in M, p in P])
@variable(production_model_det, 0<=v_import[p in P])
@variable(production_model_det, 0<=v_excess[p in P])
...
#Maximize profit
@objective(production_model_det, Max, sum(-1*investment_cost[m]*y[m] for m in M) 
            + sum(sum((sum(x[m,p] for m in M) - v_excess[p] + v_import[p])*revenue[p] for p in P))
            - sum(production_cost[p]*sum(x[m,p] for m in M) for p in P)
            - sum(import_cost[p]*v_import[p] for p in P)
            + sum(revenue_low[p]*v_excess[p] for p in P)
    )

#Max number of machines
@constraint(production_model_det, max_machines, sum(y[m] for m in M) <= maximum_machines)
#Max budget available
@constraint(production_model_det, budget, sum(investment_cost[m]*y[m] for m in M) <= maximum_budget)

#Ensure demand fulfillment
@constraint(production_model_det, demand_fulfill[p in P], sum(x[m,p] for m in M) + v_import[p] - v_excess[p] == det_demand[p])

#Limit time on machines
@constraint(production_model_det, time_limit[m in M], sum(production_time[p]*x[m,p] for p in P) <= working_time[m]*y[m])

#Ensure compatibility between machine and production
@constraint(production_model_det, compabitility[m in M, p in P], x[m,p]  <= machine_compatibility[p][m]*(working_time[m]/production_time[p])*maximum_machines)

optimize!(production_model_det)


if termination_status(production_model_det) == MOI.OPTIMAL
    ...
    println("Production $([sum(value.(x[m,p]) for m in M) for p in P])")
    println("Excess $([value.(v_excess[p]) for p in P])")
    println("Import $([value.(v_import[p]) for p in P])")
    

    @printf "\nObjective value: %0.3f\n\n" objective_value(production_model_det)
